...
describe.skipIf(!hasPgTools)("backup / restore round-trip", () => {
  ...
  it("dumps and restores knowledge entries", async () => {
    ...
    expect(spawnSync("pg_dump", ["-Fc", "-d", url(), "-f", file]).status).toBe(
      0,
    );

    await sql`delete from knowledge_entries`;
    expect(
      (await sql`select count(*)::int n from knowledge_entries`)[0].n,
    ).toBe(0);

    expect(
      spawnSync("pg_restore", ["--clean", "--if-exists", "-d", url(), file])
        .status,
    ).toBe(0);
