...
describe("core permissions", () => {
  ...
  beforeAll(async () => {
    ...
    teamAdmin = (
      await createUser({ email: "lead@example.com", role: "member" })
    ).id;
    member = (await createUser({ email: "dev@example.com", role: "member" }))
      .id;
  ...
  it("global admin can edit any scope, including org-global", async () => {
    ...
    expect(await canEditScope(globalAdmin, { productId: otherProductId })).toBe(
      true,
    );
  ...
  it("team admin edits own team by teamId and by owned product", async () => {
    ...
    expect(
      await canEditScope(teamAdmin, { productId: await tpdProductId() }),
    ).toBe(true);
  });
  ...
  it("team admin is denied cross-team and org-global scopes", async () => {
    ...
    expect(await canEditScope(teamAdmin, { productId: otherProductId })).toBe(
      false,
    );
  ...
  it("plain member is denied everywhere", async () => {
    ...
    await expect(
      assertCanEditScope(member, { teamId: testTeamId }),
...
describe("API enforcement (team mini-admin vs member vs admin)", () => {
  ...
  it("/auth/me carries the caller's team-admin set", async () => {
    ...
    expect(body.team_admin).toEqual([
      expect.objectContaining({ team_slug: "test-team" }),
    ]);
    const dev = await (
      await app.request("/auth/me", { headers: { cookie: devCookie } })
    ).json();
