...
import {
  listComponents, addComponent, saveKnowledgeEntry,
...
describe("components", () => {
  ...
  it("renames a slug and rewrites it where it tagged this product's entries", async () => {
    ...
    const tagged = await saveKnowledgeEntry({ productId: tpd, issueSummary: "x", tags: ["lc", "printing"], status: "approved" });
    const untagged = await saveKnowledgeEntry({ productId: tpd, issueSummary: "y", tags: ["printing"], status: "approved" });

    expect(await componentRenameImpact(tpd, "lc")).toEqual({ entries: 1, docs: 0 });

    const res = await renameComponent(tpd, "lc", "line-controller");
    expect(res).toMatchObject({ renamed: true, from: "lc", to: "line-controller", entries: 1 });

    expect((await listComponents(tpd)).map((c) => c.slug)).toEqual(["line-controller"]);
  ...
  it("refuses to rename onto an existing component slug in the same product", async () => {
    ...
    await expect(renameComponent(tpd, "a", "b")).rejects.toThrow(/already exists/);
  });
});
