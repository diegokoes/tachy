...
| Source tokens (Freshdesk, GitHub, ADO)     | the vault              | re-register the connection in Admin › integrations › sources with the new token, or each user's own Settings › keys                                                                | next turn and sync use the new token    |
| Provider keys (Anthropic, Copilot)         | the vault              | each user in their own Settings › keys; the deployment-wide fallback is `ANTHROPIC_API_KEY` / `COPILOT_GITHUB_TOKEN` in `.env`                                                     | next turn                               |
